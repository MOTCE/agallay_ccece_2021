...
class SwitchPeekable(messageW : Int = 8, destcodeW : Int = 8, N : Int = 32) extends Module{
  val io = IO(new Bundle{
    ...
    val fifosNValues = Output(Vec(4, UInt(log2Ceil(N+1).W)))
  ...
  val fifoUpper0 = Module(new FifoPeekable(N, messageW+destcodeW))
  val fifoUpper1 = Module(new FifoPeekable(N, messageW+destcodeW))

  val fifoLower0 = Module(new FifoPeekable(N, messageW+destcodeW))
  val fifoLower1 = Module(new FifoPeekable(N, messageW+destcodeW))
  ...
  io.fifosNValues(0) := fifoUpper0.io.nValues;
  io.fifosNValues(1) := fifoUpper1.io.nValues;
  io.fifosNValues(2) := fifoLower0.io.nValues;
  io.fifosNValues(3) := fifoLower1.io.nValues;
}
